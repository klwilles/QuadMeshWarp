#ifndef XMLMANAGER_H_WARPER
#define XMLMANAGER_H_WARPER
 ...
 *	Edited by Ben Norskov on 6.7.10
...
//      void setXMLTagFromPointer(const string & tag, float * pointer);
	...
	/************************ ADDED BY BEN */
	bool getBoolPropertyFromXMLTag(const string & tag);
	int getIntPropertyFromXMLTag(const string & tag);
	float getFloatPropertyFromXMLTag(const string & tag);
	string getStringPropertyFromXMLTag(const string & tag);
